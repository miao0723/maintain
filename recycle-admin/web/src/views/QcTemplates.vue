<template>
  <div class="qc-tpl-page">
    <div class="toolbar">
      <el-button type="primary" @click="openCreate">新建质检模板</el-button>
    </div>
      ...
      <template v-if="detail">
        <div class="section">
          ...
          <el-table :data="detail.items" border size="small">
            ...
            <el-table-column label="故障标签" min-width="150">
              <template #default="{ row }">
                <el-tag v-for="f in row.fault_options || []" :key="f" size="small" style="margin-right: 4px">{{ f }}</el-tag>
        ...
        <div class="section">
          ...
          <el-table :data="detail.grades" border size="small">
            ...
            <el-table-column prop="fault_standard" label="判定标准" min-width="220" show-overflow-tooltip />
            <el-table-column label="操作" width="120">
              <template #default="{ row }">
        ...
        <div class="section">
          ...
          <el-table :data="detail.versions" border size="small" empty-text="尚未发布过版本（请先添加质检项和成色标准并发布，质检单才能引用）">
    ...
    <el-dialog v-model="gradeVisible" :title="gradeForm.id ? '编辑成色档位' : '添加成色档位'" width="520px">
      <el-form label-width="110px">
        ...
        <el-form-item label="回收价系数">
          ...
          <span class="tip">定级建议价 = 订单价格 × 系数</span>
        </el-form-item>
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getQcTemplates({ page: pagination.page, pageSize: pagination.pageSize })
...
const openCreate = () => {
  Object.assign(editForm, { id: null, name: '', description: '' })
...
const submitEdit = async () => {
  ...
  try {
    if (editForm.id) {
      await updateQcTemplate(editForm.id, { ...editForm })
    } else {
      await createQcTemplate({ ...editForm })
    }
...
const openItemDialog = (row) => {
  if (row) {
    Object.assign(itemForm, {
      id: row.id, name: row.name, category: row.category || '', check_method: row.check_method || '',
      scoring_type: row.scoring_type, fault_options: row.fault_options || [], required: !!row.required, sort_order: row.sort_order || 0
    })
  } else {
...
const openGradeDialog = (row) => {
  if (row) {
    Object.assign(gradeForm, {
      id: row.id, grade_key: row.grade_key, grade_name: row.grade_name,
      min_score: Number(row.min_score), max_score: Number(row.max_score),
      price_coefficient: Number(row.price_coefficient), fault_standard: row.fault_standard || ''
    })
  } else {
...
const submitPublish = async () => {
  submitLoading.value = true
  try {
    const res = await publishQcVersion(detail.value.id, publishChangelog.value)
    ElMessage.success(res.message || '版本已发布')
...
<style scoped>
.qc-tpl-page .toolbar { margin-bottom: 16px; }
.qc-tpl-page .el-pagination { margin-top: 16px; justify-content: flex-end; }
.qc-tpl-page .section { margin-bottom: 24px; }
.qc-tpl-page .section-head { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; }
.qc-tpl-page .section-head h4 { margin: 0; font-size: 15px; }
.qc-tpl-page .tip { margin-left: 10px; font-size: 12px; color: #909399; }
</style>
