...
router.get('/my-orders', authenticateToken, async (req, res) => {
  try {
    ...
    let whereClause = showOnlyMine ? 'WHERE o.assigned_to = ?' : 'WHERE 1=1';
    const params = showOnlyMine ? [userId] : [];
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        total: (pendingProgress[0]?.count || 0) + (pendingOrders[0]?.count || 0) + (pendingQuote[0]?.count || 0)
      }
    });
  } catch (error) {
