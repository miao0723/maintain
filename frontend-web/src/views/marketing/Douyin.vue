...
import { createDouyin, deleteDouyin, downloadDouyin, downloadDouyinFile, generateDouyinVideo, getDouyinGenerateStatus, getDouyinList, getPublishStatus, optimizeDouyinVideo, publishDouyin, updateDouyin } from '@/api/marketing'
import { uploadFile } from '@/api/system'
const createDefaults=()=>({prompt:'',resolution:'1080p',ratio:'9:16',duration:5,watermark:false,douyin_title:'',douyin_desc:'',douyin_tags:'',target_audience:'通用本地客户',selling_points:'快速上门,透明报价,维修质保',hook_text:'',cta_text:'私信咨询，马上安排',visual_style:'真实案例风',voice_type:'女声',subtitle_style:'营销大字',bgm_style:'轻快可信',save_to_library:true})
...
const handlePublish=async row=>{try{await ElMessageBox.confirm('确定要发布该视频到抖音吗？','提示',{type:'warning'});publishing.value=true;const res=await publishDouyin(row.id);if(res.code===0){let checkCount=0;const checkStatus=async()=>{if(checkCount>=150){ElMessage.warning('发布超时，请稍后刷新查看');return}const statusRes=await getPublishStatus(row.id);if(statusRes.data.status==='success'){ElMessage.success('发布成功');fetchData()}else if(statusRes.data.status==='failed'){ElMessage.error('发布失败：'+(statusRes.data.message||'未知错误'))}else if(statusRes.data.status==='pending'||statusRes.data.status==='processing'){checkCount++;setTimeout(checkStatus,2000)}};checkStatus()}else{ElMessage.error(res.message||'发布失败')}}catch(e){if(e!=='cancel')console.error(e)}finally{publishing.value=false}}
...
const pollGenerateStatus=(id)=>new Promise(resolve=>{let count=0;const tick=async()=>{try{const res=await getDouyinGenerateStatus(id);const d=res.data||{};if(d.gen_status==='succeeded'){ElMessage.success('视频生成完成，已保存到素材库');await fetchData();resolve(true);return}if(d.gen_status==='failed'){ElMessage.error('视频生成失败：'+(d.gen_error||'未知错误'));resolve(false);return}}catch(e){console.error(e)}if(++count>=120){ElMessage.warning('生成超时（约10分钟），请稍后在素材库查看结果');resolve(false);return}setTimeout(tick,5000)};tick()})
const handleGenerate=async()=>{const valid=await generatorFormRef.value?.validate().catch(()=>false);if(!valid)return;generating.value=true;try{const res=await generateDouyinVideo(buildGeneratorPayload());const data=res.data||{};if(data.gen_status==='running'){activeTab.value='library';ElMessage.success(res.message||'生成任务已提交');await pollGenerateStatus(data.id)}else{lastGenerated.value={...(data.saved||data.draft_material||{}),downloading:false};pendingMaterial.value=data.save_to_library?null:data.draft_material;ElMessage.success(res.message||'视频创建成功');if(data.save_to_library){activeTab.value='library';await fetchData()}}}finally{generating.value=false}}
const handlePreviewVideo=row=>{if(row.video_url){window.open(row.video_url,'_blank')}}
const handleOptimize=async()=>{if(!currentOptimizeRow.value)return;optimizing.value=true;currentOptimizeRow.value.optimizing=true;try{const res=await optimizeDouyinVideo(currentOptimizeRow.value.id,{edit_config:{...optimizeForm}});const data=res.data||{};if(data.gen_status==='running'){optimizeDialogVisible.value=false;ElMessage.success(res.message||'优化任务已提交');await pollGenerateStatus(data.id)}else{const updated=data.updated_material||data.material||null;if(updated){Object.assign(currentOptimizeRow.value,updated);lastGenerated.value=updated}optimizeDialogVisible.value=false;ElMessage.success(res.message||'视频优化成功');await fetchData()}}}finally{optimizing.value=false;currentOptimizeRow.value.optimizing=false}}
