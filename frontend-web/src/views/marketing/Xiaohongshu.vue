  ...
  <div class="page">
    <div class="hero">
      <div class="hero-content">
        <p>小红书获客</p>
        <h2>图文种草 · 素材库 · 账号运营</h2>
        <span>创建图文/视频后可直接保存到素材库，素材库已预留发布入口。</span>
      </div>
      <div class="stats">
        <div class="box"><small>素材总数</small><b>{{ tableData.length }}</b></div>
        <div class="box"><small>总曝光量</small><b>{{ formatNumber(totalViews) }}</b></div>
    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="素材库" name="library">
        <el-card shadow="never">
          ...
          <el-table :data="tableData" v-loading="loading" border stripe>
            ...
            <el-table-column prop="video_url" label="内容链接" min-width="140">
              <template #default="{ row }">
                <el-link v-if="row.video_url" :href="row.video_url" target="_blank" type="primary">打开内容</el-link>
                <span v-else>暂无</span>
              </template>
            </el-table-column>
      ...
      <el-tab-pane label="内容分析" name="analysis">
        <div class="grid four">
          <div class="box"><small>累计曝光</small><b>{{ formatNumber(totalViews) }}</b></div>
          <div class="box"><small>累计点赞</small><b>{{ formatNumber(totalLikes) }}</b></div>
          <div class="box"><small>累计收藏</small><b>{{ formatNumber(totalComments) }}</b></div>
          <div class="box"><small>累计评论</small><b>{{ formatNumber(totalShares) }}</b></div>
        </div>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑素材' : '新增素材'" width="600px">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="90px">
        ...
        <el-form-item label="内容链接" prop="video_url">
          <el-input v-model="form.video_url" placeholder="请输入内容链接" />
        </el-form-item>
        <el-form-item label="封面图">
          <SingleImageUpload v-model="form.cover" placeholder="上传封面图" />
        </el-form-item>
        <el-form-item label="内容描述">
          <el-input v-model="form.description" type="textarea" :rows="3" placeholder="请输入内容描述" />
        </el-form-item>
...
const rules = {
  ...
  video_url: [{ required: true, message: '请输入内容链接', trigger: 'blur' }]
}
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getXiaohongshuList({
...
const handleDelete = async (row) => {
  try {
    await deleteXiaohongshu(row.id)
...
const handleSubmit = async () => {
  ...
  if (!valid) return
  try {
    isEdit.value ? await updateXiaohongshu(form.id, form) : await createXiaohongshu(form)
...
.hero::before {
  ...
  width: 200px;
  height: 200px;
  background: linear-gradient(135deg, rgba(255, 82, 82, 0.1), rgba(255, 137, 137, 0.05));
  border-radius: 50%;
}
