  ...
  <div class="page">
    <div class="hero">
      <div>
        <p>快手获客</p>
        <h2>视频创作 · 素材库 · 直播引流</h2>
        <span>创建视频后可直接保存到素材库，素材库已预留发布入口。</span>
      </div>
      <div class="stats">
    ...
    <el-tabs v-model="activeTab">
      ...
      <el-tab-pane label="数据统计" name="analysis">
        <div class="grid four">
          ...
          <div class="box"><small>累计评论</small><b>{{ formatNumber(totalComments) }}</b></div>
          <div class="box"><small>累计分享</small><b>{{ formatNumber(totalShares) }}</b></div>
        </div>
      </el-tab-pane>
    </el-tabs>
...
const handlePublish = async (row) => {
  try {
    await ElMessageBox.confirm('确定要发布到快手吗？', '提示', { type: 'warning' })
    const res = await publishKuaishou(row.id)
